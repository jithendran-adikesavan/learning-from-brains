...
def identify_roi(config: Dict = None) -> None:
    ...
    for batch_i, batch in enumerate(eval_dataloader):
        ...
        unembedded_data = model.unembedder.forward(logits["pooler_outputs"].view(1, 1, 768))["outputs"].detach().cpu().numpy()
        ...
        error_img_map = signals_to_img_maps(
            region_signals=unembedded_data[0][0],
            maps_img=difumo.maps
        )
        error_img_map.to_filename(os.path.join(par_dir, img_name))
        ...
        try:
            plot_brain_map(
                ...
                path=os.path.join(
                    par_dir,
                    f'mean_eval_error_{model_config["training_style"]}_{img_name}.png'
                ),
